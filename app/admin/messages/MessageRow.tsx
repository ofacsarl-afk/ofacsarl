...
export default function MessageRow({ m }: { m: Message }) {
  ...
  return (
    <div className={`rounded-2xl border bg-white p-4 ${m.is_read ? "border-zinc-200" : "border-[#c1121f]/40"}`}>
      <div className="flex items-start justify-between gap-4">
        <button className="text-left flex-1" onClick={() => { setOpen(!open); if (!m.is_read) start(() => setMessageRead(m.id, true)); }}>
          <div className="flex items-center gap-2">
            {!m.is_read && <span className="inline-block h-2 w-2 rounded-full" style={{ background: "#c1121f" }} />}
      ...
      {open && (
        <div className="mt-3">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            <a href={`mailto:${m.email}`} className="text-sm px-3 py-1.5 rounded-lg bg-[#c1121f] text-white hover:bg-[#9a0e18]">Répondre par email</a>
